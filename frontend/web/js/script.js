...
/* *********************	Misc plugins config	********************* */
	...
	if( ($().countTo) && ($('.timer').length) ) {
		...
		if (isMobileDevice) {
				$('.stats-wsection .box-content').find(".timer").countTo();
		} else {
			// appear init and then countTo
			$(".stats-wsection .box-content").appear(function() {
...
/* *********************	Website enhancement & bug fixes	********************* */
	...
	if ( $('.client').length ) {
			...
			var imgOn = $('.client.on').find('.cw-img.on').removeClass('on');
